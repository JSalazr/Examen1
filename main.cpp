...
vector<int> mezclarOrdenadamente(vector<int>v1,vector<int>v2)
{
    ...
    for(int cont=0;cont<v1.size();cont++)
    {
        respuesta.push_back(v1[cont]);
    }
    for(int cont=0;cont<v2.size();cont++)
    {
        respuesta.push_back(v2[cont]);
    }
...
public:
    ...
    {
        if((this->numero1==operador->numero1)
            && (this->numero2==operador->numero2)
            && (this->getTipo().compare(operador->getTipo())==0))
            return true;
        return false;
    }
};
